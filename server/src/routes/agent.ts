...
function buildScopedOpenAIContext(context: AgentContext, message: string, pendingOriginalMessage?: string) {
  const scope = inferQueryScope(
    pendingOriginalMessage ? `${pendingOriginalMessage} ${message}` : message,
  )
