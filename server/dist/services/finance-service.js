...
function mapExpenseAttachmentRow(row) {
    return {
        id: row.id,
        name: row.file_name,
        type: row.file_type,
...
function mapExpense(row, participants, attachments, membershipToAccount) {
    return {
        id: row.id,
...
function mapPayment(row, membershipToAccount) {
    return {
        id: row.id,
        apartmentId: row.apartment_id,
...
export async function listExpensesByApartmentId(apartmentId) {
    ...
    const [{ data: participantsData, error: participantsError }, { data: attachmentsData, error: attachmentsError }] = await Promise.all([
    ...
    if (participantsError)
        throw new Error(`Failed to load expense participants: ${participantsError.message}`);
    if (attachmentsError)
        throw new Error(`Failed to load expense attachments: ${attachmentsError.message}`);
    const participants = (participantsData ?? []);
    const attachments = (attachmentsData ?? []);
    return rows.map((row) => mapExpense(row, participants.filter((participant) => participant.expense_id === row.id), attachments.filter((attachment) => attachment.expense_id === row.id), membershipToAccount));
}
export async function createExpense(input) {
    ...
    const participantMembershipIds = input.participantAccountIds.map((accountId) => requireMembershipId(accountToMembership, accountId, 'an expense participant'));
    const { data, error } = await supabaseAdmin
        .from('expenses')
        .insert({
    ...
    if (participantMembershipIds.length) {
        const { error: participantsError } = await supabaseAdmin.from('expense_participants').insert(participantMembershipIds.map((membershipId) => ({
            expense_id: expenseRow.id,
            membership_id: membershipId,
        })));
        if (participantsError)
            throw new Error(`Failed to create expense participants: ${participantsError.message}`);
    }
    if (input.attachments?.length) {
        const { error: attachmentsError } = await supabaseAdmin.from('expense_attachments').insert(input.attachments.map((attachment) => ({
        ...
        if (attachmentsError)
            throw new Error(`Failed to create expense attachments: ${attachmentsError.message}`);
    }
...
export async function updateExpense(input) {
    ...
    if (participantMembershipIds.length) {
        ...
        if (participantsError)
            throw new Error(`Failed to update expense participants: ${participantsError.message}`);
    }
    ...
    if (input.attachments?.length) {
        ...
        if (attachmentsError)
            throw new Error(`Failed to update expense attachments: ${attachmentsError.message}`);
    }
    const expenses = await listExpensesByApartmentId(input.apartmentId);
...
export async function listPaymentsByApartmentId(apartmentId) {
    const { membershipToAccount } = await loadMembershipMaps(apartmentId);
    const { data, error } = await supabaseAdmin
        .from('payments')
        .select('*')
    ...
    return (data ?? []).map((row) => mapPayment(row, membershipToAccount));
}
...
export async function softDeletePayment(paymentId) {
    const { error } = await supabaseAdmin
        .from('payments')
        .update({
        status: 'cancelled',
